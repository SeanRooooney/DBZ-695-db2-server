    ...
    struct sqludf_dbinfo *dbinfo)
{
    ...
    {
        strcheck = sprintf(cmdstring, "pgrep -fx \"DB2DIR/bin/asncap capture_schema=%s capture_server=%s\" > %s", asnService, dbname, tmpFileName);
        int callcheck;
        ...
        if (strcmp(asnCommand, "start") == 0)
        {
            ...
            {
                strcheck = sprintf(cmdstring, "DB2DIR/bin/asncap capture_schema=%s capture_server=%s &", asnService, dbname);
            ...
            (strcmp(asnCommand, "stop") == 0))
        {
            if (len > 0)
            {
                ...
                //callcheck = system(cmdstring);
                strcheck = sprintf(cmdstring, "DB2DIR/bin/asnccmd capture_schema=%s capture_server=%s %s >> %s", asnService, dbname, asnCommand, tmpFileName);
